    ...
    fun parse(input: TINYScanner): Tree {
        ...
        while (!nextFocus.isEmpty()) {
            ...
            if (variables.contains(focus.term)) { // foco é NT
                ...
                if (focus.term == Token.tokenName(lookAhead.tipo)) {
                    lookAhead = input.token()
                } else {
                    throw RuntimeException("erro de sintaxe em " + lookAhead.lin +
                            ", input: " + lookAhead + ", foco: " +
                            focus.term)
                }
            }
        }
